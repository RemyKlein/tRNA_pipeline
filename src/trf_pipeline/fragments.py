...
def generate_fragments(
    ...
    grouped: dict[str, list[FragmentOrigin]] = defaultdict(list)
    for trna in sorted(trnas, key=lambda item: item.locus.identifier):
        genomic_mature_end = (1 if trna.histidine_minus_one else 0) + len(trna.genomic_sequence)
        ...
        for start in range(len(trna.sequence)):
            for length in range(minimum, min(maximum, len(trna.sequence) - start) + 1):
                ...
                grouped[trna.sequence[start:end]].append(
                    FragmentOrigin(
                        ...
                        trna.locus.strand,
                        start + (0 if trna.histidine_minus_one else 1),
                        end - (0 if trna.histidine_minus_one else 1),
